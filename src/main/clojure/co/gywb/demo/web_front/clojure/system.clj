...
(gen-class
  ...
  :prefix "-"
  :main false
  :methods [[^{GET true Path "/start-nrepl" Produces ["text/plain"]}
             startNrepl [] String]
            [^{GET true Path "/stop-nrepl" Produces ["text/plain"]}
             stopNrepl [] String]
            [^{Inject true} setDataSource
             [javax.sql.DataSource] void]
            [^{Inject true} setVertx
             [io.vertx.axle.core.Vertx]
             void]])
...
(defn -startNrepl [_]
  (if (:nrepl-server @system-state)
    "Already running"
    (do (swap! system-state assoc :nrepl-server (start-nrepl))
        "Started")))

(defn -stopNrepl [_]
  (if-let [nrepl-server (:nrepl-server @system-state)]
    (do (stop-server nrepl-server)
        (swap! system-state assoc :nrepl-server nil)
        "Stopped")
    "Not running"))

(defn -setDataSource [_ ^DataSource datasource]
  (swap! system-state assoc :datasource datasource))

(defn -setVertx [_ ^Vertx vertx]
  (swap! system-state assoc :vertx vertx)
  nil)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
